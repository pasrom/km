...
Install a release by its commit (`git ls-remote https://github.com/pasrom/km refs/tags/v1.0.3` shows
it), not by the tag: `pip install git+https://github.com/pasrom/km@<commit>  # v1.0.3`, or run it
without installing: `uvx --from git+https://github.com/pasrom/km@<commit> km validate  # v1.0.3`.
...
```yaml
- uses: pasrom/km@<commit> # v1.0.3, installs the km CLI at that commit
- run: km validate
- run: km gen-index --check
```

The pre-commit hook works the same way (`repo: https://github.com/pasrom/km`,
`rev: <commit>  # frozen: v1.0.3`, `id: km-validate`). Every pin names a release by its commit, so a
